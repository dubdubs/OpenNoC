...
module tb_axi2chi_nocoh_txn_ctx_child;
  ...
  initial begin
    ...
    @(posedge clk);
    @(negedge clk);
    `CHECK(dut.child_q[0].axi_byte_offset == 0);
    `CHECK(dut.child_q[0].line_byte_offset == 6'h00);
    `CHECK(dut.child_q[0].fragment_byte_count == 1);
    ...
    // AXI R retirement of beat 0 releases only child 0; child 1 remains
    // allocated until its own beat retires.
    axi_beat_retire_parent_idx_i = '0;
    axi_beat_retire_beat_i = '0;
    axi_beat_retire_all_i = 1'b0;
    axi_beat_retire_valid_i = 1'b1;
    @(posedge clk);
    @(negedge clk);
    axi_beat_retire_valid_i = 1'b0;
    #1;
    ...
    @(posedge clk);
    @(negedge clk);
    `CHECK(!dut.child_q[0].valid);
    `CHECK(dut.child_q[1].valid);
    child_alloc_valid_i = 1'b1;
    #1;
    `CHECK(child_alloc_ready_o && child_alloc_idx_o == 1'd0);
    `CHECK(child_alloc_txnid_o == 3'd0);
