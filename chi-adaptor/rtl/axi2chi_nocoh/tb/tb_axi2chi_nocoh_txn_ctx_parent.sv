...
module tb_axi2chi_nocoh_txn_ctx_parent;
  ...
  localparam int unsigned MaxAxiBeats = 256;
  ...
  logic child_alloc_last_fragment_o;
  logic [$clog2(128 / 8 + 1)-1:0] child_alloc_axi_byte_offset_o;
  logic [$clog2(64)-1:0] child_alloc_line_byte_offset_o;
  logic [$clog2(128 / 8 + 1)-1:0] child_alloc_fragment_byte_count_o;
  logic child_release_valid_i;
  logic child_release_ready_o;
  logic [$clog2(ChildEntries)-1:0] child_release_idx_i;
  logic axi_beat_retire_valid_i;
  logic [$clog2(ParentEntries)-1:0] axi_beat_retire_parent_idx_i;
  logic [AxlenWidth-1:0] axi_beat_retire_beat_i;
  logic axi_beat_retire_all_i;
  ...
  logic [AxlenWidth-1:0] child_lookup_axi_beat_o;
  logic [1:0] child_lookup_frag_idx_o;
  logic child_lookup_last_fragment_o;
  logic [$clog2(128 / 8 + 1)-1:0] child_lookup_axi_byte_offset_o;
  logic [$clog2(64)-1:0] child_lookup_line_byte_offset_o;
  logic [$clog2(128 / 8 + 1)-1:0] child_lookup_fragment_byte_count_o;
  logic parent_retire_valid_i;
  logic [$clog2(ParentEntries)-1:0] parent_retire_idx_i;
  logic parent_retire_ready_o;
  logic parent_retire_query_valid_i;
  logic [$clog2(ParentEntries)-1:0] parent_retire_query_idx_i;
  logic parent_retire_query_permit_o;
  logic [ParentEntries-1:0] parent_retire_permit_vec_o;
  logic parent_lookup_valid_i;
  logic [$clog2(ParentEntries)-1:0] parent_lookup_idx_i;
  logic [AxiIdWidth-1:0] parent_lookup_axi_id_o;
  ...
  initial begin
    ...
    rd_admit_id_i = 4'h1;
    rd_admit_addr_i = 64'h1000;
    rd_admit_len_i = 8'd0;
    rd_admit_size_i = 3'd4;
    rd_admit_burst_i = 2'b01;
    wr_admit_valid_i = 1'b0;
    wr_admit_id_i = 4'h2;
    wr_admit_addr_i = 64'h2000;
    wr_admit_len_i = 8'd1;
    wr_admit_size_i = 3'd4;
    wr_admit_burst_i = 2'b01;
    ...
    axi_beat_retire_valid_i = 1'b0;
    axi_beat_retire_parent_idx_i = '0;
    axi_beat_retire_beat_i = '0;
    axi_beat_retire_all_i = 1'b0;
    parent_lookup_valid_i = 1'b0;
    parent_lookup_idx_i = '0;
    ...
    parent_retire_query_valid_i = 1'b0;
    parent_retire_query_idx_i = '0;
    parent_retire_idx_i = '0;
    rd_issue_ready_i = 1'b0;
    wr_issue_ready_i = 1'b0;

    #1;
    `CHECK(!rd_admit_ready_o && !wr_admit_ready_o);

    @(negedge clk);
    rst = 1'b0;
    rd_admit_valid_i = 1'b1;
    wr_admit_valid_i = 1'b1;
    #1;
    `CHECK(rd_admit_ready_o && !wr_admit_ready_o);
    ...
    #1;
    `CHECK(!rd_admit_ready_o && wr_admit_ready_o);
    `CHECK(wr_admit_parent_idx_o == 1'd1);
    ...
    parent_retire_idx_i = 1'd0;
    parent_retire_valid_i = 1'b1;
    #1;
    `CHECK(parent_retire_ready_o);
    ...
    #1;
    `CHECK(!rd_admit_ready_o && !wr_admit_ready_o);
    $display("PASS: parent context allocation, arbitration, and release");
    $finish;
  end
endmodule
