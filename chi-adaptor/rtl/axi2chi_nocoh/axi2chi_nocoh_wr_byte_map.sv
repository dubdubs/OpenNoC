// Maps one AXI byte range into a CHI data payload and byte-enable vector.

`default_nettype none

module axi2chi_nocoh_wr_byte_map #(
  parameter int unsigned AxiDataWidth = 128,
  parameter int unsigned ChiDataWidth = 256,
  parameter int unsigned CacheLineBytes = 64
) (
  input logic [AxiDataWidth-1:0] axi_data_i,
  input logic [AxiDataWidth / 8-1:0] axi_strb_i,
  ...
  output logic [ChiDataWidth-1:0] chi_data_o,
  output logic [ChiDataWidth / 8-1:0] chi_be_o
);
