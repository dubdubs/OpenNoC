...
module rni_arctrl
    ...
    reg [7:0] arctrl_entry_policy_epoch_q[RNI_AR_ENTRIES_NUM_PARAM-1:0];
    ...
    rni_arlink rni_arlink_u0
               (
                   ...
                   ,.arlink_lock_s2_o              (                       )
                   ,.arlink_profile_coherent_o     (arlink_profile_coherent_w)
                   ,.arlink_policy_epoch_o         (arlink_policy_epoch_w  )
    ...
    generate
        for (entry=0; entry < RNI_AR_ENTRIES_NUM_PARAM; entry=entry+1) begin: txn_info
            ...
            always @(posedge clk_i or posedge rst_i) begin
                if (rst_i == 1'b1) begin
                    arctrl_entry_profile_q[entry] <= 1'b1;
                    arctrl_entry_policy_epoch_q[entry] <= 8'b0;
                end else if (arctrl_alloc_ptr_s1_w[entry]) begin
                    arctrl_entry_profile_q[entry] <= arlink_profile_coherent_w;
                    arctrl_entry_policy_epoch_q[entry] <= arlink_policy_epoch_w;
                end
            end
    ...
    always@* begin
        ...
        begin
            ...
            ar_txreq_profile_r = ar_txreq_profile_r | (arctrl_entry_req_ptr_q[i] & arctrl_entry_profile_q[i]);
        end
        ar_txreq_txnid_r[`CHIE_REQ_FLIT_TXNID_WIDTH-1] = ar_txreq_profile_r;
        ar_txreq_txnid_r[`CHIE_REQ_FLIT_TXNID_WIDTH-2] = 1'b0;
    end
    ...
    always@* begin
        ...
        ar_txreqflit_info_r[`CHIE_REQ_FLIT_OPCODE_RANGE] = ar_txreq_profile_r ? `CHIE_READONCE : `CHIE_READNOSNP;
        ar_txreqflit_info_r[`CHIE_REQ_FLIT_ALLOWRETRY_RANGE] = ar_txreq_profile_r & ~arctrl_entry_req_select_retry_flag_q;
        ...
        ar_txreqflit_info_r[`CHIE_REQ_FLIT_MEMATTR_CACHEABLE_RANGE] = ar_txreq_profile_r;
        ar_txreqflit_info_r[`CHIE_REQ_FLIT_SNPATTR_RANGE] = ar_txreq_profile_r;
    ...
    assign ar_rxrsp_correct_w = rxrspflitv_d1_i &
                                ~arctrl_entry_rxrsp_txnid_w[`CHIE_RSP_FLIT_TXNID_WIDTH-2] &
                                (arctrl_entry_rxrsp_txnid_w[`CHIE_RSP_FLIT_TXNID_WIDTH-1] ==
                                 arctrl_entry_profile_q[arctrl_entry_rxrsp_txnid_w[`RNI_AR_ENTRIES_WIDTH-1:0]]) &
                                (arctrl_entry_rxrsp_tgtid_w[`CHIE_RSP_FLIT_TGTID_WIDTH-1:0] == RNI_NID_PARAM);
