...
module rni_awctrl `RNI_PARAM
    ...
    input  wire                                         aw_profile_coherent_i;
    input  wire [7:0]                                   aw_policy_epoch_i;
    output wire                                         aw_outstanding_o;
    ...
    output  reg [`CHIE_DAT_FLIT_DBID_WIDTH-1:0]         awctrl_txdat_original_txnid_d2_o;
    ...
    input   wire                                        awctrl_txdat_sent_d3_i;
    ...
    wire                                        awlink_profile_coherent_w;
    wire [7:0]                                  awlink_policy_epoch_w;
    wire                                        awlink_pending_w;
    ...
    reg                                         awctrl_entry_profile_q [RNI_AW_ENTRIES_NUM_PARAM-1:0];
    reg [7:0]                                   awctrl_entry_policy_epoch_q [RNI_AW_ENTRIES_NUM_PARAM-1:0];
    ...
    rni_awlink rni_awlink_u0
               (
                   ...
                   ,.AWVALID                      (AWVALID0               )
                   ,.AWBUS                        (AW_CH_S0               )
                   ,.profile_coherent_i           (aw_profile_coherent_i  )
                   ,.policy_epoch_i               (aw_policy_epoch_i      )
                   ...
                   ,.awlink_lock_s2_o             ()
                   ,.awlink_profile_coherent_o    (awlink_profile_coherent_w)
                   ,.awlink_policy_epoch_o        (awlink_policy_epoch_w  )
                   ,.awlink_pending_o             (awlink_pending_w       )
               );

    assign aw_outstanding_o = awlink_pending_w | (|awctrl_entry_v_q);
    ...
    generate
        for (entry=0; entry < RNI_AW_ENTRIES_NUM_PARAM; entry=entry+1) begin: txn_info
            ...
            // Capture policy on AW acceptance.  The AXI4 W channel has no
            // transaction ID, so every W beat must inherit this snapshot.
            always @(posedge clk_i or posedge rst_i) begin
                if (rst_i == 1'b1) begin
                    awctrl_entry_profile_q[entry] <= 1'b0;
                    awctrl_entry_policy_epoch_q[entry] <= 8'b0;
                end else if (awctrl_alloc_ptr_s1_w[entry]) begin
                    awctrl_entry_profile_q[entry] <= awlink_profile_coherent_w;
                    awctrl_entry_policy_epoch_q[entry] <= awlink_policy_epoch_w;
                end
            end
    ...
    // CompAck is a coherent-profile capability.  A non-coherent DBID/DAT/Comp
    // sequence must never inherit an ExpCompAck dependency from an older AW.
    assign aw_txreq_expcompack_w = awctrl_new_entry_compack_dep_w &
                                   awlink_profile_coherent_w;
    ...
    // txreq select
    /////////////////////////////////////////////////////////////
    assign awctrl_req_retry_ready_w[RNI_AW_ENTRIES_NUM_PARAM-1:0] = awctrl_entry_v_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] & wdata_recv_done_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] & awctrl_entry_req_select_rdy_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] & rxrsp_pcrdgrant_recv_vec_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] & ~awctrl_entry_req_select_vec_q[RNI_AW_ENTRIES_NUM_PARAM-1:0];
    ...
    // Do not issue a write request before its W data set has been collected.
    // The legacy done bit is the default-width compatibility form of the
    // parameterized fragment-finalize boundary.
    assign awctrl_req_new_rdy_w[RNI_AW_ENTRIES_NUM_PARAM-1:0] =
        awctrl_entry_v_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] &
        wdata_recv_done_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] &
        awctrl_entry_req_select_rdy_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] &
        ~awctrl_entry_req_dep_v_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] &
        ~rxrsp_retryack_recv_vec_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] &
        ~awctrl_entry_req_select_vec_q[RNI_AW_ENTRIES_NUM_PARAM-1:0];
    ...
    always@* begin
        ...
        for (i=0; i < RNI_AW_ENTRIES_NUM_PARAM; i=i+1) begin
            ...
            aw_txreq_profile_r = aw_txreq_profile_r |
                (awctrl_entry_req_ptr_q[i] & awctrl_entry_profile_q[i]);
        end
        // Unified namespace: {profile, direction(write), slot}.
        aw_txreq_txnid_r[`CHIE_REQ_FLIT_TXNID_WIDTH-1] = aw_txreq_profile_r;
        aw_txreq_txnid_r[`CHIE_REQ_FLIT_TXNID_WIDTH-2] = 1'b1;
    end
    ...
    always@* begin
        ...
        aw_txreqflit_info_r[`CHIE_REQ_FLIT_OPCODE_RANGE] = aw_txreq_profile_r ? `CHIE_WRITEUNIQUEPTL : `CHIE_WRITENOSNPPTL;
        aw_txreqflit_info_r[`CHIE_REQ_FLIT_ALLOWRETRY_RANGE] = aw_txreq_profile_r & ~awctrl_entry_req_select_retry_flag_q;
        ...
        aw_txreqflit_info_r[`CHIE_REQ_FLIT_MEMATTR_CACHEABLE_RANGE] = aw_txreq_profile_r;
        aw_txreqflit_info_r[`CHIE_REQ_FLIT_SNPATTR_RANGE] = aw_txreq_profile_r;
    ...
    assign aw_rxrsp_correct_w = awctrl_rxrspflitv_d1_i &
                                awctrl_entry_rxrsp_txnid_w[`CHIE_RSP_FLIT_TXNID_WIDTH-2] &
                                (awctrl_entry_rxrsp_txnid_w[`CHIE_RSP_FLIT_TXNID_WIDTH-1] ==
                                 awctrl_entry_profile_q[awctrl_entry_rxrsp_txnid_w[`RNI_AW_ENTRIES_WIDTH-1:0]]) &
                                (awctrl_entry_rxrsp_tgtid_w[`CHIE_RSP_FLIT_TGTID_WIDTH-1:0] == RNI_NID_PARAM);
    ...
    // DAT completion is a raw-link event. txdat_select_vec_d3_q filters the
    // first half of a two-DAT line, so only the final DAT handshake may
    // unblock CompAck/B scheduling.
    assign txdat_send_vec_ns_w[RNI_AW_ENTRIES_NUM_PARAM-1:0] =
        (txdat_send_vec_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] |
         ({RNI_AW_ENTRIES_NUM_PARAM{awctrl_txdat_sent_d3_i}} &
          txdat_rdy_entry_d3_q[RNI_AW_ENTRIES_NUM_PARAM-1:0] &
          txdat_select_vec_d3_q[RNI_AW_ENTRIES_NUM_PARAM-1:0])) &
        ~awctrl_entry_dealloc_vec_w[RNI_AW_ENTRIES_NUM_PARAM-1:0];
    ...
    // Preserve the exact TxnID used by the selected AW request.  TXDAT uses
    // this value in its DBID field; rebuilding it in the data buffer would
    // lose the profile bit from the unified {profile, direction, slot} ID.
    always@* begin
        awctrl_txdat_original_txnid_d2_o =
            {`CHIE_DAT_FLIT_DBID_WIDTH{1'b0}};
        for (i=0; i < RNI_AW_ENTRIES_NUM_PARAM; i=i+1) begin
            if(txdat_rdy_v_d2_q && txdat_rdy_entry_d2_q[i]) begin
                awctrl_txdat_original_txnid_d2_o[`RNI_AW_ENTRIES_WIDTH-1:0] = i;
                awctrl_txdat_original_txnid_d2_o[`CHIE_DAT_FLIT_DBID_WIDTH-1] =
                    awctrl_entry_profile_q[i];
                awctrl_txdat_original_txnid_d2_o[`CHIE_DAT_FLIT_DBID_WIDTH-2] =
                    1'b1;
            end
        end
    end
    ...
    always @(posedge clk_i or posedge rst_i) begin
        if (rst_i == 1'b1)begin
            ...
            if(awctrl_txdat_sent_d3_i | awctrl_entry_dealloc_v_w)begin
