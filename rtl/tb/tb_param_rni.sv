...
module tb_param_rni;
  ...
  always_ff @(posedge clk) begin
    ...
    if (!rst && tx_req_flit_valid &&
        tx_req_flit[`CHIE_REQ_FLIT_OPCODE_RANGE] == `CHIE_READONCE) begin
      read_seen <= 1'b1;
      read_txnid <= tx_req_flit[`CHIE_REQ_FLIT_TXNID_RANGE];
      if (tx_req_flit[`CHIE_REQ_FLIT_TXNID_MSB] != 1'b1 ||
          tx_req_flit[`CHIE_REQ_FLIT_TXNID_MSB-1] != 1'b0) begin
        $error("ReadOnce TxnID=%h did not use coherent/read encoding",
               tx_req_flit[`CHIE_REQ_FLIT_TXNID_RANGE]);
        failure_count <= failure_count + 1;
      end
    end
    if (!rst && tx_req_flit_valid &&
        tx_req_flit[`CHIE_REQ_FLIT_OPCODE_RANGE] == `CHIE_WRITEUNIQUEPTL) begin
      write_seen <= 1'b1;
      write_txnid <= tx_req_flit[`CHIE_REQ_FLIT_TXNID_RANGE];
      if (tx_req_flit[`CHIE_REQ_FLIT_TXNID_MSB] != 1'b1 ||
          tx_req_flit[`CHIE_REQ_FLIT_TXNID_MSB-1] != 1'b1) begin
        $error("WriteUniquePtl TxnID=%h did not use coherent/write encoding",
               tx_req_flit[`CHIE_REQ_FLIT_TXNID_RANGE]);
        failure_count <= failure_count + 1;
      end
    end
