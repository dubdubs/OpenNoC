...
module tb_param_rni_full;
    ...
    wire [1:0]                                  BRESP0;
    wire [10:0]                                 RID0;
    wire [AXI4_AXDATA_WIDTH_PARAM-1:0]          RDATA0;
    wire [1:0]                                  RRESP0;
    reg                                         TXRSPFLITV;
    reg                                         TXRSPLCRDV;
    reg                                         TXDATFLITV;
    ...
    reg                                         TXLINKACTIVEACK;
    reg                                         RXLINKACTIVEREQ;
    integer                                     feature_error_count;
    integer                                     readonce_count;
    integer                                     writeuniqueptl_count;
...
// Initialization
    ...
    // Functional checks for the two request opcodes generated by RNI.
    // The legacy sequences below provide many AXI shapes; this monitor makes
    // their CHI request type and endpoint mapping observable and checked.
    always @(posedge clk) begin
        if (!rst && TXREQFLITV &&
            (TXREQFLIT[`CHIE_REQ_FLIT_OPCODE_RANGE] != `CHIE_REQLCRDRETURN)) begin
            if (TXREQFLIT[`CHIE_REQ_FLIT_SRCID_RANGE] != RNI_NID_PARAM ||
                TXREQFLIT[`CHIE_REQ_FLIT_TGTID_RANGE] != HNF_NID_PARAM) begin
                feature_error_count = feature_error_count + 1;
                $display("ERROR: RNI request has unexpected SourceID or TargetID");
            end
            case (TXREQFLIT[`CHIE_REQ_FLIT_OPCODE_RANGE])
                `CHIE_READONCE: begin
                    readonce_count = readonce_count + 1;
                    if (TXREQFLIT[`CHIE_REQ_FLIT_SIZE_RANGE] != 3'b110) begin
                        feature_error_count = feature_error_count + 1;
                        $display("ERROR: ReadOnce Size is not the implemented 64-byte value");
                    end
                end
                `CHIE_WRITEUNIQUEPTL:
                    writeuniqueptl_count = writeuniqueptl_count + 1;
                default: begin
                    feature_error_count = feature_error_count + 1;
                    $display("ERROR: RNI emitted unsupported request opcode %h",
                             TXREQFLIT[`CHIE_REQ_FLIT_OPCODE_RANGE]);
                end
            endcase
        end

        if (!rst && RVALID0) begin
            axi_read_beat_count = axi_read_beat_count + 1;
        end
        if (!rst && BVALID0) begin
...
`ifdef AR_TEST_EN
    ...
    initial begin
        ...
        r_entry_v <= 0;;
        forever @(posedge clk) begin
            if(TXREQFLITV && (TXREQFLIT[`CHIE_REQ_FLIT_OPCODE_RANGE] == `CHIE_READONCE))begin
...
`ifdef AW_TEST_EN
    ...
    initial begin
        ...
        w_entry_v <= 0;
        forever @(posedge clk) begin
            if(TXREQFLITV && (TXREQFLIT[`CHIE_REQ_FLIT_OPCODE_RANGE] == `CHIE_WRITEUNIQUEPTL))begin
...
// Result check
    ...
    initial begin
        ...
        repeat (50) @(posedge clk);
        if (feature_error_count != 0 || readonce_count == 0 ||
            writeuniqueptl_count == 0 || axi_read_beat_count == 0 ||
            axi_write_response_count == 0) begin
            $fatal(1, "RNI functional checks failed: errors=%0d ReadOnce=%0d WriteUniquePtl=%0d R=%0d B=%0d",
                   feature_error_count, readonce_count, writeuniqueptl_count,
                   axi_read_beat_count, axi_write_response_count);
        end
        $display("\n\n\
        #####################################################\n\
        test_result:PASSED  #################################\n\
